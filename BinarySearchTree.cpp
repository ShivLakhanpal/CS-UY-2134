...
public:
    ...
    if(root != NULL){
      bstQueue.push(root);
      levelOrderTraversal(root, bstQueue);

      while(bstQueue.size() != 0){
      cout << bstQueue.front()->element << " ";
      bstQueue.pop();
      }
    }
  }

  void levelOrderTraversal(Node* top, queue<Node*>& binaryQueue) const{
  if (top->left != nullptr)
    binaryQueue.push(top->left);
  if (top->right != nullptr)
    binaryQueue.push(top->right);
  if (top->left != nullptr)
    levelOrderTraversal(top->left, binaryQueue);
  if (top->right != nullptr)
    levelOrderTraversal(top->right, binaryQueue);
}
    ...
    double d = -1;
    double nod = 0;
    average_node_depth(root, sum, d, nod);
    return (sum / nod);
  }

  void average_node_depth(Node* top, double& sum, double d, double& nod) const{
  if (top != nullptr){
    d++;
    nod++;
    sum += d;
    average_node_depth(top->left, sum, d, nod);
    average_node_depth(top->right, sum, d, nod);
  }
	...
	void stringy(){
		vector<Node*> bstVec;
		stringy(bstVec, root);
		for (int i = 0; i < bstVec.size() - 1; i++)
		{
			bstVec[i]->left = NULL;
			bstVec[i]->right = bstVec[i + 1];
		}
		bstVec[bstVec.size() - 1]->left = NULL;

		swap(root, bstVec[0]);
	}
	void stringy(vector<Node*>& v, Node* top)
	{
		if (top->left != NULL) stringy(v, top->left);
		v.push_back(top);
		if (top->right != NULL) stringy(v, top->right);
	}
...
private:
    ...
    //rangedPrint function
void rangedPrint(int low, int high, Node* top) const{
  if (top == NULL)
    return;
  else if (top->element == low){
    cout << top->element << " ";
    rangedPrint(low, high, top->right);
  }
  else if (top->element == high){
    rangedPrint(low, high, top->left);
    cout << top->element << " ";
    return;
  }
  else if (top->element < high && top->element >low){
    rangedPrint(low, high, top->left);
    cout << top->element << " ";
    rangedPrint(low, high, top->right);
  }
  else
    if (top->element > high) rangedPrint(low, high, top->left);
    else
      return;
}
